...
docker exec lea_api_$CONT touch "$DIR/lea_connect_$VER.aab"
echo "Created $DIR/lea_connect_$VER.aab"
docker exec lea_api_$CONT ls -la "$DIR"

curl -X POST "http://127.0.0.1:$ADM_PORT/build/scanBuildAppAndroid"